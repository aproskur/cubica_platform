...
$('#static-container').on('click', '.archive-link', function (e) {
  e.preventDefault();
  if (e.target) {
    let text = "Вы действительно хотите отправить ссылку в архив?"
...
$(document).ready(function() {
  ...
  $('#static-container').on({
    "mouseover" : (e) => {
      let buttons =  $(e.target).closest('.main-form').find('.launch-controls');
      let selects =  $(e.target).closest('.main-form').find('.date-launches');
      let row =  $(e.target).closest('.main-form').find('.launch-link-generator');
  ...
  $('#static-container-archive').on({
    "mouseover" : (e) => {
        let row = $(e.target).closest('.archive-links-container');
        let buttons = $(e.target).closest('.archive-links-container').find('.button-arch-copy');
        row.css("border", "1px solid var(--theme-yellow)");
        buttons.removeClass("nodisplay");
