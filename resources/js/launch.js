...
$(document).ready(function() {
  ...
  $('#static-container').on('click', '.archive-link', function(e){
    ...
    e.preventDefault();
    if(e.target){
      const link_id = $(e.target).closest('.main-form').find('.link-id').val();
      console.log("DELETE DATA"+link_id);
      let gameId= $("#game_id").val();
    $.ajaxSetup({
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  ...
  //DELETE
    $('#static-container').on('click', '.delete-link', function(e){
      ...
      $.ajax({
        ...
        data: {"id": link_id , _method: 'delete'},
        dataType: "json",
        success: function(){
          //поменять hide на другой метод
          $(e.target).closest('.main-form').hide();
        ...
        $('#static-container').on('click', '.gear', function(e) {
          ...
          $(e.target). animate(
              { deg: 180 },
              {
                  duration: 1000,
                  step: function(now) {
                    $(e.target). css({ transform: 'rotate(' + now + 'deg)' });
                  }
              });
